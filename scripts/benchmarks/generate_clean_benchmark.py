...
OUT_DIR = Path("benchmark_configs/clean_only")
...
for arch_label, arch in ARCHS.items():
    for ds_label, ds in DATASETS.items():
        for method_label, method in METHODS.items():
            ...
            cfg["experiment_name"] = f"{ds['name']}_{arch['model']}_{method['method_name']}_clean_seed1"
            cfg["dataset"].update({
                "name": ds["name"],
...
manifest = Path("outputs/benchmark_results_clean/clean_suite_manifest.json")
manifest.parent.mkdir(parents=True, exist_ok=True)
manifest.write_text(json.dumps(rows, indent=2), encoding="utf-8")
print(f"Generated {len(rows)} clean configs")
print(manifest)
